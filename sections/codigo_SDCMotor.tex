...
El archivo \texttt{SDCMotor.cpp} implementa una \emph{S-Function Level-2} para Simulink, cuyo objetivo es actuar como puente entre el modelo de control y un módulo de E/S analógicas OPTO22 mediante Ethernet. Para ello, utiliza la librería \texttt{O22SIOMM} y la clase \texttt{O22SnapIoMemMap}. En cada paso de simulación, el bloque:
\begin{itemize}
    \item \textbf{envía} al OPTO22 dos comandos analógicos: un comando de voltaje para el actuador y una corriente de carga (\texttt{iLoad})\footnote{El significado físico exacto depende de la conexión real.};
...
La S-Function abre la comunicación Ethernet con un \emph{Brain} OPTO22 en la IP \texttt{192.168.6.101} y el puerto \texttt{2001}. Por lo tanto, el computador y el equipo OPTO22 deben estar en la misma red o subred, sin bloqueo por firewall o ACL. Estos valores están embebidos en \texttt{SDCMotor.cpp}; si se cambia el setup, deben actualizarse en el código o parametrizarse.
...
\end{itemize}
MATLAB permite especificar rutas de \emph{include} con \texttt{-I} y opciones de enlazado durante la compilación con \texttt{mex}.

\subsubsection{Compilación recomendada desde MATLAB}
Desde la carpeta donde reside \texttt{SDCMotor.cpp}, se recomienda compilar con:
\begin{verbatim}
mex -v SDCMotor.cpp ...
...
\end{verbatim}
y se encapsula en \texttt{extern C} para asegurar un \emph{linkage} compatible con el mecanismo de carga de Simulink.
...
\begin{itemize}
    ...
    \]
\end{itemize}
Si la cantidad de parámetros provistos en Simulink no coincide con lo esperado, \texttt{mdlInitializeSizes()} retorna y Simulink reporta el error correspondiente.
...
Para mantener información entre llamadas, la S-Function reserva:
\begin{itemize}
...
\begin{enumerate}
    ...
    \end{itemize}
    \item Espera a que la apertura finalice consultando \texttt{IsOpenDone()} mientras retorne \texttt{SIOMM\_ERROR\_NOT\_CONNECTED\_YET}.
...
\paragraph{Nota.}
El mensaje impreso ante error en el segundo envío repite el texto \texttt{Error sending voltage command.\textbackslash n}. Para mejorar la trazabilidad, se recomienda diferenciar los mensajes de error asociados al comando de voltaje y a la corriente de carga.
...
\begin{enumerate}
    ...
    \item Se recomienda enviar \textbf{0.0} al canal analógico 1 (\texttt{SetAnaPtValue(1,0.0)}) para asegurar que el comando de carga también quede en cero al finalizar.
...
\begin{figure}
    ...
    \caption{Diagrama funcional de la S-Function \texttt{SDCMotor}.}
    \label{fig:SDCMotor diagram}
\end{figure}

\subsection{Consideraciones de integración y configuración}
\begin{itemize}
    \item \textbf{IP del computador host:} el computador debe configurarse con una dirección compatible con la subred del OPTO22, evitando conflictos con otros adaptadores o redes activas.
    ...
    \item \textbf{Entradas analógicas adicionales:} en la configuración actual del setup, solo se ocupan 2 de los 4 puntos de entrada del módulo \texttt{SNAP-AIV-4} \cite{opto22_snap_aiv4}. Habilitar puntos adicionales puede incrementar el \emph{overhead} de comunicación y dificultar el uso de tiempos de muestreo menores.
